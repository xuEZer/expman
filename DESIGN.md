...
每卡 Gate 维护上次启动时间和等待退出状态。默认内存门槛 0.1、启动间隔 5 秒，定义于 devices 模块，显存与主机内存共用同一门槛常量。空闲比例大于等于门槛时，一次增加一个实验。小于门槛时 kill 最新启动进程组并阻止补位；其他实验任意退出重新开放检查，自身用于减载的退出不开放；空卡可在满足门槛和间隔后重启。各卡独立，显存仍不足时可以继续减载。GPU UUID 在连续采样间变化、设备缺失或显存观测失败均终止调度并清理子进程，避免使用错误或陈旧数据继续启动。

主机内存是每个实验进程共用的资源，单独由全局 HostGate 把关，采样 `/proc/meminfo` 的 `MemTotal`、`MemAvailable`，并同时读取 swap 供外部观察。`MemAvailable` 比例低于门槛时，所有卡都停止派发，并按最近启动顺序 kill 一个运行中的实验；减载后 HostGate 进入阻止补位状态，直到某个实验以非压力原因退出（成功、失败或中断）才重新开放准入，没有运行实验时只由内存比例决定。减载仍按尝试处理：退出记为 cancelled，不占失败预算，实验回到队列等待重试。swap 不参与任何准入或减载判定。显存与主机内存各自独立判定，同一次采样中两者都触发时会在同一轮内各减载一次。主机内存读数失败与显存查询失败同路径处理：暂停派发并有限重试，连续失败终止调度。
...
GPU 显存查询使用内部默认超时 10 秒；查询失败或返回无效数据时清除旧显存读数，暂停新任务派发，等待 1 秒后在调度循环重试。重试之间继续收集已有 worker 的事件和退出结果，成功查询清零连续失败计数。连续 3 次失败时明确报错，并按调度中断流程结束剩余 worker、保存恢复信息。主机内存读数与显存查询在同一轮采样、共用同一失败计数：任一路径失败都清除旧读数并暂停派发，`/proc/meminfo` 缺失、字段不全或数值无效都会报错，不使用可能造成误减载的近似值。GPU 身份改变属于不可恢复的设备一致性错误。上述参数由框架内部定义。
