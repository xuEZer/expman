...
文档根也允许是 `!choice`，候选为完整配置映射，用于让同级字段联动（数据集 A 绑定模型 a、数据集 B 绑定模型 b），而不是对各字段取笛卡尔积：

```yaml
!choice
- {dataset: {name: A}, imputer: {name: a}}
- {dataset: {name: B}, imputer: {name: b}}
```

根候选仍可包含嵌套 `!choice` 和 `name` 默认；每个候选按普通文档展开与合并。候选不是映射（标量、列表、`null`）报错。根 `!choice` 与顶层 `sweep` 互斥。

### 7.2 参数敏感性扫描

实验文件的顶层 `sweep` 段把同一条配置展开成 OFAT（one-factor-at-a-time）敏感性扫描。文件自身的字段是主实验基线，`axes` 用点号路径声明要扫的轴，其余轴保持基线值：
...
sweep:
  ...
  axes:
    tip.subspace_rank: [2, 4]
    tip.epsilon: [0.00001, 0.001]
```

- `ofat`（默认）逐个轴变化、其他轴固定在基线，Run 数为各轴长度之和再加上基线条目；`grid` 改为轴值的笛卡尔积。顺序为基线、再按 `axes` 书写顺序逐轴逐值。
- 轴路径必须在实验文件中已经存在；路径经过 `!choice` 节点会报错，避免在未选择的分支上声明依赖。轴值覆盖 `name` 默认文件中的同名参数。
- `sweep` 是框架保留的顶层键，展开前移除，不进入 Run 配置；默认文件出现 `sweep` 报错。
- `sweep` 与 `!choice` 正交：扫描变体仍按 7.1 展开候选，再按 7.3 合并默认值。
...
- 实验文件和默认文件都要求单个 YAML 文档、字符串键；重复字段和未知标签报错。实验文件顶层可以是映射或映射候选的 `!choice`，默认文件只允许映射顶层。
- 默认文件中任何位置的 `!choice` 都报错，即使对应参数会被实验配置覆盖；默认文件也不允许 `sweep`。
