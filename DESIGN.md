...
执行时先恢复队首的取消成员，其余新成员按时间估计的信息价值选取；失败重试成员保留队尾顺序。成功则完成；任意普通异常记录失败，并在重试预算未耗尽时放到队尾。默认 `max_retries=1` 表示最多两次尝试；该参数必须为非负整数。

例如信息调度先后选中 A、B、C，A 首次失败而 B 和 C 成功，则尝试顺序为 `A1 → B1 → C1 → A2`。A2 再失败就标记最终失败。Stage 构造异常采用同一策略，单个实验失败不终止整组。
...
`Batch.run(progress=True, refresh_interval=1.0)` 默认启动后台显示线程，在标准错误流定期输出实验完成数、百分比、成功数、最终失败数、当前实验编号、已运行时间及剩余时间区间。已运行时间使用单调时钟统计本次 `run()` 的实际经过时间，按 `DD:HH:MM` 向下取整到分钟，结束或中断时固定计时；恢复调用重新计时。等待重试的失败不计入已完成。终端宽度足够时原地刷新，否则输出完整行；重定向输出不含控制字符并抑制重复状态。CPU 业务执行保留在调用线程，GPU 调度运行在调用线程；结束或中断时停止显示线程并输出最终状态。显示或估计异常不影响实验执行；估计异常时显示问号。

`Batch.estimate(coverage=None) -> TimeEstimate` 提供整个 Batch 的剩余执行时间区间。覆盖参数默认 0.8，由构造参数 `estimate_coverage` 配置并写入 Batch 清单；调用时可临时覆盖。时间统一显示为 `DD:HH:MM～DD:HH:MM`，下界向下、上界向上取整到分钟，天数超过两位不截断。无有效成功样本时为 `??:??:??～??:??:??`；队列及活动成员均结束时为零。

### 观测与恢复

Experiment 通过单调时钟测量整个尝试，运行中的起始时刻仅保存在内存中，线程锁保护尝试和活动耗时的快照。估计器使用每个实验的累计已知尝试耗时。成功实验贡献一个完整观测；待运行、重试或运行中的实验在累计耗时大于零时贡献右删失观测。最终失败的实验既不进入成功样本集，也不继续占用剩余队列。每个实验至多对应一个统计观测，不把各次尝试作为互相独立的实验。

观测从现有尝试清单恢复，配置使用保存的完整配置。`InterruptedRun` 标识未知耗时的硬退出；该实验之后成功也不会用已知的局部耗时替代完整时间训练模型。已知的累计时长仍可在待执行期间作为下界。长时间没有完成的实验影响条件预测，不通过减去一个固定点估计获得剩余量。后台监控可以读取估计，CPU 业务执行保持在调用线程；GPU 业务由隔离子进程执行，中断规则见下节。

### 统计模型

自动展开配置的嵌套映射/序列叶子，忽略恒定字段；数值做带符号 log1p 变换和归一化，类别值独热编码。内部特征超过 48 维时使用确定性带符号散列，限制矩阵计算规模；散列碰撞和无关配置字段可能降低预测能力。另加常数项，不依赖用户字段类型声明或业务循环。

对整个实验的累计执行秒数使用对数正态回归：`log(T) = xβ + ε`，`ε ~ Normal(0, σ²)`。采用 Normal–Inverse-Gamma 正则化先验：截距精度 1e-6，其余系数精度 1，均值为 0，方差先验 shape=2、scale=1。这些是内部统计正则化，未取得成功样本前不输出绝对时间范围。

完整数据的后验采用共轭更新；右删失观测通过截断正态潜变量进行 Gibbs 数据增强。默认预热 192 次、保留 768 个联合预测样本，使用局部确定性随机数生成器，不改变用户训练随机状态。每个样本共享回归参数和噪声尺度，再为各待执行实验抽取条件耗时；有累计耗时的实验要求总耗时大于该下界，再减去已运行量。顺序执行下累加各成员剩余量，最后对整批样本取中心分位数，保留公共模型参数造成的相关性。

共轭公式依据 [Bayesian linear regression posterior](https://statproofbook.github.io/P/blr-post)；删失观测的概率处理参照 [Stan censored data](https://mc-stan.org/docs/2_27/stan-users-guide/censored-data.html)。实现只依赖 Python 标准库。区间是模型下的近似后验预测区间，有限采样和分布假设不保证实际覆盖率为 80%；`calibrated` 保持 False，真实工作负载验证仍需要积累数据。数值上界溢出时使用未知端点，不截成有限时长。
...
每卡 Gate 只维护 `gpu_block`，没有显存比例门槛：显存读数无法预测下一次分配，缺多少只在失败时才知道。一个尝试以 CUDA OOM 结束（`torch.cuda.OutOfMemoryError`，或信息匹配 `CUDA out of memory`／`CUDA error: out of memory`／`CUDNN_STATUS_ALLOC_FAILED`）时置该卡 `gpu_block`，此后不再往这张卡新增实验，直到该卡上有尝试正常结束（成功或非 OOM 失败）才清除。封禁不阻塞该卡上已经在跑的尝试；该卡已无运行尝试时仍允许启动一个试探，否则单卡上唯一一个 OOM 会让批次永久停住。每个 tick 每卡最多启动一个实验，没有启动间隔，也没有每卡并发上限。GPU UUID 在连续采样间变化或设备缺失属于不可恢复的设备一致性错误，终止调度并清理子进程，避免使用错误数据继续启动。

主机内存是每个实验进程共用的预算，由全局 HostGate 的 `mem_block` 与准入预留共同把关。观测来自 `/proc/meminfo`（`MemTotal`、`MemAvailable`、swap）与 `/proc/vmstat`（`pswpin`/`pswpout` 的区间增长，代表内核**正在**换页，而不是 swap 占用）。`MemAvailable` 低于固定预留 2 GiB，或区间内发生换页时置 `mem_block`：所有卡停止派发，并 kill 全局最近启动的一个实验，读数仍不足时每个 tick 继续减载一个；减载自身不清除 `mem_block`，直到某个实验自然退出。

峰值不再按 run_id 查表，而是**按配置特征估计**：用与耗时模型同一套 `features()`（展开整份配置、数值带符号 log 变换并归一化、类别独热、超过 48 维确定性散列）做对数正态贝叶斯回归，取 0.9 上分位作为该 run 的峰值估计（`_memory_model.PeakEstimator`）；观测是 `_gpu_history` 的 `peak_kb`，被 cgroup 限额打断的尝试作为右删失观测（只知下界），并把该 run 的估计地板抬到 `实测 peak × 1.5`。冷启动为 `HOST_PEAK_KB_DEFAULT`（1 GiB）——一条观测都没有时整批都按它计费——有观测后未跑过的 run 由回归给出估计，最坏被 `peak_ceiling`（已观测最大值）兜住。准入按预留量算而不是快照：要求 `headroom ≥ 本次估计 + Σ max(worker 峰值, 该 run 估计) − worker 当前驻留`，其中 worker 驻留与峰值来自它自己的 cgroup（回退到 `/proc/<pid>/status` 的 `VmRSS`/`VmHWM`）。不满足时把已选中的 run 放回队首，本 tick 不再尝试任何卡。只要估计是真实峰值的上界，所有 worker 的峰值之和就不会超过预算，这正是限制并发的机制；估算高只损失吞吐。

每个 worker 还跑在按估计设置的 cgroup 限额里：`memory.max = max(512 MiB, 估计 × 1.25)` 与 `memory.swap.max = 0`，优先直接写 cgroupfs 并把子进程加入该 cgroup，`/sys/fs/cgroup` 只读时回退到 `systemd-run --user --scope --property=MemoryMax=...`（`--scope` 是 exec 替换，stdin EOF 看护、进程组 kill 与 pid 语义都保持）。只用 `memory.max`：`memory.high` 对没有可回收对象的匿名分配只会把进程节流到停住。限额被驳回（`memory.events` 的 `max`/`oom_kill` 增长，或峰值逼近限额）时，worker 内的监视线程把这条证据落盘（写在被杀之前），父进程每 tick 采样 cgroup 作为兜底；该尝试记为 `cancelled`/`MemoryLimit`，不占失败重试预算，并按抬高后的估计重试，跑完的尝试即使触及限额也照常保留结果。
...
主进程定期记录各运行实验经历的同卡并发数时间积分，结束后保存平均并发、设备编号/UUID 和经过时间。GPU ETA 沿用整实验累计耗时观测与删失处理，将设备和并发条件加入回归；每次联合抽样产生各成员剩余量，在当前每卡槽位下模拟队列，输出最晚完成时间的中心预测区间。空闲卡只有在既没有 `gpu_block`、也没有主机内存压力时获得一个预测槽位；不预测未来升降并发轨迹。主机读取为 `tight` 或 `admits_next` 为假时预测只保留已占用的槽位，与压力期间停止派发和减载的准入规则一致；没有实验在运行且主机内存不足时剩余时间未知。设备迁移和并发变化用汇总特征近似，不能识别所有 GPU 内竞争、模型加载和外部负载影响；区间仍未校准。信息价值选择保留原有实验配置启发式，准入由主机内存预留与显存 OOM 封禁共同决定；显存侧没有需求估计（由真实分配失败兜住），主机侧的需求估计来自配置特征回归，两者都随观测更新。
