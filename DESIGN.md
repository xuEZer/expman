...
每卡 Gate 维护上次启动时间和等待退出状态。默认显存门槛 0.1、启动间隔 5 秒，定义于 devices 模块。空闲比例大于等于门槛时，一次增加一个实验。小于门槛时 kill 最新启动进程组并阻止补位；其他实验任意退出重新开放检查，自身用于减载的退出不开放；空卡可在满足门槛和间隔后重启。各卡独立，显存仍不足时可以继续减载。GPU UUID 在连续采样间变化、设备缺失或显存观测失败均终止调度并清理子进程，避免使用错误或陈旧数据继续启动。
...
主进程定期记录各运行实验经历的同卡并发数时间积分，结束后保存平均并发、设备编号/UUID 和经过时间。GPU ETA 沿用整实验累计耗时观测与删失处理，将设备和并发条件加入回归；每次联合抽样产生各成员剩余量，在当前每卡槽位和启动间隔下模拟队列，输出最晚完成时间的中心预测区间。空闲卡仅在余量满足门槛时获得一个预测槽位；不预测未来升降并发轨迹。设备迁移和并发变化用汇总特征近似，不能识别所有 GPU 内竞争、模型加载和外部负载影响；区间仍未校准。信息价值选择保留原有实验配置启发式，GPU 准入仅由显存决定。
