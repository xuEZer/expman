...
`Batch.run(progress=True, refresh_interval=1.0)` 默认启动后台显示线程，在标准错误流定期输出实验完成数、百分比、成功数、最终失败数、当前实验编号、已运行时间及剩余时间区间。已运行时间使用单调时钟统计本次 `run()` 的实际经过时间，按 `DD:HH:MM` 向下取整到分钟，结束或中断时固定计时；恢复调用重新计时。等待重试的失败不计入已完成。终端宽度足够时原地刷新，否则输出完整行；重定向输出不含控制字符并抑制重复状态。业务执行保留在调用线程，结束或中断时停止显示线程并输出最终状态。显示或估计异常不影响实验执行；估计异常时显示问号。
...
观测从现有尝试清单恢复，配置使用保存的完整配置。`InterruptedRun` 标识未知耗时的硬退出；该实验之后成功也不会用已知的局部耗时替代完整时间训练模型。已知的累计时长仍可在待执行期间作为下界。长时间没有完成的实验影响条件预测，不通过减去一个固定点估计获得剩余量。后台监控可以读取估计，实际业务执行保持在主线程，Ctrl+C 的原有停止与恢复语义保持有效。
...
当前执行环境是 `sequential`，估计配置版本和环境类型保存到清单，旧清单默认按顺序执行处理，未知版本/环境拒绝加载。模型仅使用当前 Batch 的记录，不自动跨机器校准；外部资源竞争、未来额外失败、停机/人工等待以及批量调度本身的开销未单独建模。动态并发和抢占调度需要在后续版本提供实际并行观测后接入。
