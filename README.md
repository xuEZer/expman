...
GPU 调度按 Stage 的滚动依赖特征直接选择候选：重试优先保留，其他资源可行候选选择与该 Stage 已采样配置最远的一项。模型只在新的 Stage 完成 IPC 到达时更新；CLI 仍读取最近有效的 Batch ETA，后台计时线程统一刷新默认覆盖率的预测并保存。并行 ETA 的历史记录按 run_id 索引，避免每个实验扫描整份历史。
