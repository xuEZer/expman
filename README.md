...
调度默认值在 [devices.py](src/expman/devices.py) 中：内存裕量比例 `MEMORY_MARGIN = 0.01`（1%）、显存查询超时 `QUERY_TIMEOUT = 3.0` 秒、启动间隔 `LAUNCH_INTERVAL = 5.0` 秒；它们不是 YAML 调度字段。每个调度 tick（`POLL_INTERVAL = 0.2` 秒）同时重新查询每张卡的整卡空闲显存（`nvidia-smi`，包含外部程序占用）和主机内存（`/proc/meminfo` 的 `MemAvailable`）：显存决定每张卡能跑多少，主机内存是所有实验进程共用的上限。该机制只作用于 GPU 调度，`device: []` 的顺序执行路径不受影响。

准入由两个阻塞标志控制，且只在实验自然退出时释放。

显存（每张卡一个 `gpu_block`）：

- 空闲比例至少 1%、该卡没有 `gpu_block`（或该卡已无运行实验），且距离上次启动至少 5 秒时，尝试在该卡启动一个实验；没有固定并发数量上限。
- 空闲比例低于 1% 时置该卡的 `gpu_block`，并 kill 这张卡上最近启动的实验；读数仍不足时每个 tick 继续减载一个。
- 该卡有实验自然退出（成功、失败或中断）时清除 `gpu_block`；减载 kill 自身不清除，因此不会立即补位。该卡已无运行实验时，满足比例和启动间隔即可启动。
- 启动间隔不保证覆盖模型加载时间，显存裕量也不是下一个实验的需求估计，仍可能发生 OOM。实际失败按原规则获得一次队尾重试；调度中断不消耗失败重试次数。

主机内存（全局 `mem_block`，所有卡共用一个门槛）：

- `MemAvailable` 比例低于 1% 时置 `mem_block`，任何卡都不再启动新实验，并 kill 全局最近启动的一个实验；读数仍不足时每个 tick 继续减载一个。
...
- 这些门槛都不是下一个实验的需求估计。1% 是极宽松的默认值：8 GiB 卡上约合 82 MiB，32 GiB 主机上约合 320 MB，也就是接近耗尽时才开始减载，实际 OOM 余量留给用户；提高 `MEMORY_MARGIN` 可以更早停止派发。资源长期不足时，框架会反复减载并重试；单个实验本身就放不下时，该批次无法取得进展，只能看到取消与重试。
