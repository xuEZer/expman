...
`Batch.estimate()` 的公开剩余时间区间仍以整个实验为单位，自动提取完整配置中的变化字段并使用实际耗时学习；GPU 调度器另外按顶层 Stage 维护资源和耗时估计。没有有效完成样本时显示问号；已有完成样本后输出模型预测区间。尚未完成的实验提供“累计运行至少这么久”的信息，避免只用先完成的短实验。指标和进度上报频率不影响估时接入。
...
模型按 Stage 分开保存。特征是滚动累积的配置依赖：某 Stage 的上游 Stage 已读取路径与它自身已观测的读取路径共同决定特征。因此前缀依赖相同表示上游输入相同。冷启动时使用完整配置挑选彼此更远的组合；有样本后，参数距离只在同一 Stage 内用于候选排序。耗时、主机内存峰值和显存峰值模型只在收到新的完成样本时重建，tick 不会用正在运行的瞬时数据改变估计。
