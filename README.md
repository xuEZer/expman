...
调度默认值在 [devices.py](src/expman/devices.py) 中：内存裕量比例 `MEMORY_MARGIN = 0.1`，启动间隔 `LAUNCH_INTERVAL = 5.0` 秒；它们不是 YAML 调度字段。每张卡独立采样整卡空闲显存（包含外部程序占用），主进程另外采样主机内存（`/proc/meminfo` 的 `MemAvailable`）：显存决定每张卡能跑多少，主机内存是所有实验进程共用的上限。

显存（每卡独立）：

- 空闲比例至少 10%，且距离上次启动至少 5 秒时，尝试启动一个实验；没有固定并发数量上限。
- 空闲比例低于 10% 时，停止启动，直接 kill 该卡上最近启动的实验；后续观测仍不足时可以继续降低并发。
...
- `MemAvailable` 比例低于 10% 时，任何卡都不再启动新实验，并按最近启动顺序 kill 一个运行中的实验，不分卡；观察到的比例仍不足时会继续减载。
...
- 主机内存读数失败与显存查询失败同样处理：暂停派发并有限重试，连续 3 次失败则中断调度并清理子进程。
- 这个门槛同样不是下一个实验的需求估计。主机内存长期不足时，框架会反复减载并重试；单个实验本身就放不下时，该批次无法取得进展，只能看到取消与重试。
...
GPU 执行目前支持 Linux/WSL，需要可查询选定设备显存的 NVIDIA 驱动、`nvidia-smi` 和可读的 `/proc/meminfo`；观测失败会停止调度、清理本批次进程并报错。Stage 应定义为可导入模块或入口脚本的顶层类，入口使用 `if __name__ == "__main__":`；Pipeline 和自定义 Serializer 需要可通过标准 pickle 传入新解释器。子进程 stdin 用于检测主进程退出，不支持交互式输入。保存器的对象/设备恢复语义保持原约定；建议把需要在主进程汇总的结果转换为 CPU 对象，避免反序列化结果时在主进程占用 GPU。
...
GPU 显存查询使用内部默认超时 10 秒；查询失败或返回无效数据时清除旧显存读数，暂停新任务派发，等待 1 秒后在调度循环重试。重试之间继续收集已有 worker 的事件和退出结果，成功查询清零连续失败计数。连续 3 次失败时明确报错，并按调度中断流程结束剩余 worker、保存恢复信息。GPU 身份改变属于不可恢复的设备一致性错误。上述参数由框架内部定义。
