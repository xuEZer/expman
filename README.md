...
- `MemAvailable` 低于 1 GiB 预留，或自上次采样以来内核发生过换页（`/proc/vmstat` 的 `pswpin`/`pswpout` 增长）时置 `mem_block`，任何卡都不再启动新实验，并 kill 全局最近启动的一个实验；读数仍不足时每个 tick 继续减载一个。
