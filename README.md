...
在实验 YAML 顶层指定可使用的 NVIDIA GPU 编号（以 `nvidia-smi` 为准）：
...
调度默认值在 [devices.py](src/expman/devices.py) 中：内存裕量比例 `MEMORY_MARGIN = 0.1`、内存观测间隔 `MEMORY_QUERY_INTERVAL = 5.0` 秒、启动间隔 `LAUNCH_INTERVAL = 5.0` 秒；它们不是 YAML 调度字段。每张卡独立采样整卡空闲显存（包含外部程序占用），主进程另外采样主机内存（`/proc/meminfo` 的 `MemAvailable`）：显存决定每张卡能跑多少，主机内存是所有实验进程共用的上限。两者每 5 秒一起重新观测一次，采样之间沿用最近一次读数做准入判断；实验进程的事件收集和退出检测仍按 `POLL_INTERVAL = 0.2` 秒进行。该机制只作用于 GPU 调度，`device: []` 的顺序执行路径不受影响。

显存（每卡独立）：

- 空闲比例至少 10%，且距离上次启动至少 5 秒时，尝试启动一个实验；没有固定并发数量上限。
- 空闲比例低于 10% 时，该卡停止启动，并直接 kill 该卡上最近启动的实验。
- 降并发后等待该卡其他实验进程退出，成功、失败或中断退出都可重新触发检查。用于降并发的 kill 退出不立即补位；如果该卡没有运行实验，则满足显存比例及启动间隔后允许启动。
- 启动间隔不保证覆盖模型加载时间，显存裕量也不是下一个实验的需求估计，仍可能发生 OOM。实际失败按原规则获得一次队尾重试；调度中断不消耗失败重试次数。

主机内存（全局，所有卡共用同一个门槛）：

- `MemAvailable` 比例低于 10% 时，任何卡都不再启动新实验，并 kill 全局最近启动的一个实验；下一次观测仍不足时继续减载。
- 内存压力减载后不立即补位：只要还有实验在运行就保持停止派发，直到某个实验自然退出（成功、失败或中断）才重新开放准入；没有运行实验时只由内存比例决定。
- 每个观测轮次最多停一个实验：主机内存不足时停全局最近启动的那个，显存不足时停该卡最近启动的那个。
- swap 只观测、不参与准入和减载判定；`Batch._host_memory` 同时发布 `available_ratio`、`available_kb`、`total_kb` 和 `swap_free_kb`，便于外部观察。
- 显存或主机内存任一路径查询失败都按资源紧张处理：发出 `RuntimeWarning`，暂停所有派发并停掉最近启动的一个实验，到下一次观测再重新查询。失败不会中断批次；`nvidia-smi` 或 `/proc/meminfo` 长期不可用时，批次保持等待并重复告警。
...
Batch 清单只由主进程更新。指标仍同步写入同一个 SQLite 数据库，跨进程写入等待锁最长 30 秒。Recorder 事件通过每次尝试的事件文件转交主进程，单进程事件保持顺序，不承诺跨进程的全局发生顺序；强制退出时尚未完整写入的事件可能丢失。子进程的 stdout/stderr 位于 `experiments/<run_id>/attempts/<attempt>/output.log`，CLI 由主进程统一输出各卡运行数量、整体进度及时间。

GPU 执行目前支持 Linux/WSL，需要可查询选定设备显存的 NVIDIA 驱动、`nvidia-smi` 和可读的 `/proc/meminfo`；任一路径查询失败都按资源紧张处理，暂停派发并减载，不会中断本批次。Stage 应定义为可导入模块或入口脚本的顶层类，入口使用 `if __name__ == "__main__":`；Pipeline 和自定义 Serializer 需要可通过标准 pickle 传入新解释器。子进程 stdin 用于检测主进程退出，不支持交互式输入。保存器的对象/设备恢复语义保持原约定；建议把需要在主进程汇总的结果转换为 CPU 对象，避免反序列化结果时在主进程占用 GPU。
...
配置读取采用保守规则：读取 `ctx.cfg["data"]` 就记录整个 data 子树，即使后面只用其中一个字段；列表同样处理。遍历整个配置会记录整个配置。配置中的字段必须显式声明，`get()` 和 `in` 判断抛出 TypeError，索引不存在的字段抛出 KeyError。state 仍是普通字典。checkpoint 保存截至该时刻的配置读取依赖，恢复时合并到当前阶段追踪器，阶段完成时使用中断前后的完整读取集合匹配缓存。只有缺少依赖记录的旧 checkpoint 才保守地依赖完整配置。
...
GPU 显存查询使用内部默认超时 10 秒，显存与主机内存的观测间隔默认为 5 秒。查询失败或返回无效数据时清除旧读数、发出 `RuntimeWarning`、暂停派发并减载一次，下一轮观测重新查询，调度不因查询失败终止。GPU 身份改变属于不可恢复的设备一致性错误。上述参数由框架内部定义。
...
GPU 派发读取后台已发布的任务信息价值评分，并按本次已派发配置增量更新多样性距离。新完成样本出现后，后台重新计算评分；等待评分期间仍可启动实验。模型拟合与全队列评分在 Batch 调度锁之外执行。CLI 只读取最近有效 ETA，后台计时线程统一刷新默认覆盖率的预测并保存，避免每次终端刷新重复采样。显式调用 `Batch.estimate()` 仍即时计算；后台评分尚未就绪时使用多样性选择。并行 ETA 的历史记录按 run_id 索引，避免每个实验扫描整份历史。
