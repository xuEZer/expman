...
- cgroup 上限随 Stage 内存估计应用 110% 保守系数；受限 Stage 抬升自身峰值下界后重试。可选 PyTorch 分配器采用每进程显存比例上限；显存报告为零的已采样 Stage 自动进入 CPU 调度通道，不要求 Stage 声明资源类型。

- CUDA OOM 后的 GPU 阻塞状态持久化，并在同卡 worker 正常结束前拒绝新增派发。
