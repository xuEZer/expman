...
- 调度准入同时观察逐卡显存与主机内存：两者每 5 秒一起观测一次，采样之间沿用最近一次读数，低于门槛时每个观测轮次最多减载一个实验（主机内存不足或查询失败停全局最近启动的那个，显存不足停该卡最近启动的那个），减载后直到有实验自然退出才补位；swap 只观测；查询失败按资源紧张处理，不再中断调度；并发 ETA 在压力期间只保留已占用的槽位。
